...
- **Feb 16, 2026:** Fan Gifts & Artist Wallet System: Schema (artist_gifts, artist_wallets, wallet_transactions). Fans can donate from $1 to $1,000 with thank-you messages. Stripe Elements card input for real payment processing. 10% platform fee, 90% goes to artist. Artist wallet with atomic balance updates (db.transaction). Server-side PaymentIntent verification before crediting. APIs: POST /api/artists/:id/gift, POST /api/gifts/:id/confirm, GET /api/artist/gifts, GET /api/public/artists/:id/gifts-summary. ArtistProfilePage has pink-themed donation form with preset amounts ($1-$50) + custom amount. ArtistDashboardPage has new "Gifts & Wallet" tab showing balance, gift history, transaction ledger. Gift summary section on public artist profiles shows recent gifts.
- **Feb 16, 2026:** SupportChat service context: Chat component accepts serviceContext prop, shows context badge and hint. App.tsx derives context from current route (create, library, studio, sample_lab, artist_dashboard, discover, etc.) and passes to SupportChat. Sends context to backend for scoped AI responses. Full i18n (es/en).
- **Feb 16, 2026:** Artist Profile Social Features & Discography Enhancement: Schema additions (artist_profile_likes, artist_profile_comments, artist_profile_shares tables + youtubeUrls/spotifyArtistId fields on artist_profiles). Artist-owned discography CRUD routes (8 routes: album/track CRUD, Spotify search/import). Profile social interaction routes (7 routes: like toggle, comments CRUD, share tracking). ArtistDashboardPage: New "Discography" tab with Spotify import workflow and album management. ArtistProfilePage: Enhanced with social features (likes, comments, share buttons for Twitter/Facebook/WhatsApp/link), YouTube embeds from artist.youtubeUrls, integrated discography with album grid and Spotify preview playback. Full i18n (es/en).
- **Feb 16, 2026:** DigitalOcean GPU Droplet Integration: Created `scripts/gpu_droplet_setup.sh` (system setup) and `scripts/dgb_training_server.py` (FastAPI training server) for DO GPU Droplet (1 GPU/80GB VRAM/20 vCPU/240GB RAM). Training server provides: `/api/health` (GPU status), `/api/upload-instrument` (audio analysis with librosa), `/api/train-kit` (SAO fine-tuning with webhook callbacks), `/api/models` (list/download trained models). Added DO droplet to cloud_servers DB (priority 10, capabilities: instrument_processing, training, audio_analysis). Updated training pipeline in routes.ts: now falls back to cloud_servers table when RunPod is unavailable, trying DigitalOcean or any active server with "training" capability. Same fallback for instrument analysis route. RunPod server (id:2) deactivated.