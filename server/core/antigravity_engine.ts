...
const GENRE_INSTRUMENT_MAP: Record<string, string> = {
  "bachata": "acoustic guitar arpeggios, bongo drums, güira scraping percussion, electric bass, romantic Latin feel, Dominican bachata rhythm, 130 BPM",
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a music production prompt engineer. Convert user music requests into concise English descriptions optimized for AI music generation models like MusicGen.

Rules:
- Output ONLY the English music description, nothing else
- Describe the sound: instruments, tempo, mood, rhythm pattern, genre characteristics  
- Keep under 200 characters
- Never include lyrics or singing instructions
- Focus on instrumental and production qualities
