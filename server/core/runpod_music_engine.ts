...
try:
    ...
    # === FALLBACK: Try Stable Audio Open if HeartMuLa fails ===
    print("[HeartMuLa] Attempting SAO fallback on same GPU...")
    try:
        import torch
        import torchaudio
        
        device = "cuda" if torch.cuda.is_available() else "cpu"
        print(f"[SAO Fallback] Device: {device}")
        
        sao_wav = os.path.join(output_dir, f"sao_song_{song_id}_{int(time.time())}.wav")
        sao_mp3 = sao_wav.replace(".wav", ".mp3")
        sao_duration = min(duration_ms // 1000, 47)
        
        try:
            from stable_audio_tools import get_pretrained_model
            from stable_audio_tools.inference.generation import generate_diffusion_cond
            
            print("[SAO Fallback] Loading Stable Audio Open model...")
            model, model_config = get_pretrained_model("stabilityai/stable-audio-open-1.0")
            sample_rate = model_config["sample_rate"]
            sample_size = model_config["sample_size"]
            model = model.to(device)
            ...
            conditioning = [{"prompt": prompt, "seconds_start": 0, "seconds_total": sao_duration}]
            
            print(f"[SAO Fallback] Generating {sao_duration}s audio...")
            gen_start = time.time()
            with torch.no_grad():
            ...
            if output.dim() == 1:
                output = output.unsqueeze(0)
            torchaudio.save(sao_wav, output, sample_rate)
            gen_time = time.time() - gen_start
            print(f"[SAO Fallback] Generated WAV in {gen_time:.1f}s")
            
            final = sao_wav
            try:
                r = subprocess.run(["ffmpeg", "-i", sao_wav, "-codec:a", "libmp3lame", "-b:a", "192k", "-y", sao_mp3], capture_output=True, text=True, timeout=60)
                if r.returncode == 0 and os.path.exists(sao_mp3):
                    final = sao_mp3
            except:
                pass
            ...
            is_mp3 = final.endswith(".mp3")
            resp = requests.post(webhook_url, json={
                "songId": song_id, "status": "completed", "audioBase64": audio_b64,
                "audioFormat": "mp3" if is_mp3 else "wav", "sampleRate": sample_rate,
                "duration": sao_duration, "generationTime": round(gen_time, 1),
                "fileSize": file_size, "device": device, "engine": "sao_fallback"
            }, timeout=120, headers={"Content-Type": "application/json"})
            print(f"[SAO Fallback] Webhook response: {resp.status_code}")
            ...
            print("[SAO Fallback] stable_audio_tools not available, trying diffusers...")
            try:
                from diffusers import StableAudioPipeline
                pipe = StableAudioPipeline.from_pretrained("stabilityai/stable-audio-open-1.0", torch_dtype=torch.float16 if device == "cuda" else torch.float32)
                pipe = pipe.to(device)
                gen_start = time.time()
                audio = pipe(prompt, negative_prompt="low quality, distorted", num_inference_steps=100, audio_end_in_s=sao_duration, num_waveforms_per_prompt=1, guidance_scale=7.0).audios
                import soundfile as sf
                out_audio = audio[0].T if audio[0].ndim > 1 else audio[0]
                sf.write(sao_wav, out_audio, 44100)
                gen_time = time.time() - gen_start
                ...
                resp = requests.post(webhook_url, json={
                    "songId": song_id, "status": "completed", "audioBase64": audio_b64,
                    "audioFormat": "mp3" if is_mp3 else "wav", "sampleRate": 44100,
                    "duration": sao_duration, "generationTime": round(gen_time, 1),
                    "fileSize": file_size, "device": device, "engine": "diffusers_fallback"
                }, timeout=120, headers={"Content-Type": "application/json"})
                print(f"[Diffusers Fallback] Webhook response: {resp.status_code}")
                del pipe
                if device == "cuda":
                    torch.cuda.empty_cache()
            except Exception as e3:
                print(f"[SAO Fallback] All fallbacks failed: {e3}")
                send_error(f"HeartMuLa: {str(e)} | SAO fallback: {str(e3)}")
        except Exception as e2:
            print(f"[SAO Fallback] SAO generation failed: {e2}")
            send_error(f"HeartMuLa: {str(e)} | SAO: {str(e2)}")
