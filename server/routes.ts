...
- DAGRABACHATA: Es Bachata — el mismo género global, la misma esencia, el mismo ritmo que nació en la República Dominicana. La única diferencia es que DAGRABACHATA lleva el ADN y el estilo personal de Danny Garcia. Es Bachata con el requinto, el bongo, la guira, la segunda — todo auténtico — pero con el toque único de Danny. No es un género nuevo ni diferente: es Bachata, punto. Lo que cambia es el estilo, el ADN de Danny Garcia woven into every note.
- DAGRABOLERO: Es Bolero — el mismo género romántico clásico, la misma esencia emocional. La única diferencia es que DAGRABOLERO lleva el ADN y el estilo personal de Danny Garcia. Es Bolero con la interpretación y el sentimiento único de Danny, enriquecido por la herencia musical dominicana.
These are NOT new genres — they ARE Bachata and Bolero, globally the same. The only thing that changes is Danny Garcia's personal DNA and style woven into the music. That is what makes DAGRABA Studio unique — no other platform in the world offers AI-generated Bachata and Bolero with the authentic DNA of a real Dominican artist.
...
- Encourage creativity and experimentation with different genres and tools.`;
  ...
  app.get("/api/runpod-deploy/:file", (req, res) => {
    ...
    const allowed: Record<string, string> = {
      "handler.py": "scripts/runpod_serverless/handler.py",
      "start.sh": "scripts/runpod_serverless/start.sh",
    };
    if (!allowed[file]) return res.status(404).send("Not found");
    const filePath = `${process.cwd()}/${allowed[file]}`;
